...
- YOLO11 Pose/RF-DETR Keypoint Preview official pretrained weight로 사람 pose 영상 시각화와 좌우 비교
...
- COCO JSON 기반 전달용 dataset packaging
...
사람 pose inference 비교 notebook은 아래 폴더에 있다.
...
Label Studio와 직접 관련 없는 dataset packaging notebook은 아래 폴더에 있다.
...
1. `01_coco_pack_images_labels.ipynb`: COCO bbox JSON의 이미지 경로와 annotation을 이용해 전달용 `images/`, `labels/`, `annotations/` dataset 폴더 생성

dataset tools notebook도 기본값은 `DRY_RUN=True`, `RUN_PACK=False`로 시작한다.

자세한 notebook 운영 정책은 [docs/notebook-workflow-guide.md](docs/notebook-workflow-guide.md)를 본다. 2차 고급 workflow는 [docs/workflow-labelstudio-advanced.md](docs/workflow-labelstudio-advanced.md)를 본다. 영상 frame 추출은 [docs/video-frame-extraction-guide.md](docs/video-frame-extraction-guide.md)를 본다. YOLO11/RF-DETR 영상 inference 비교는 [docs/video-inference-visualization-guide.md](docs/video-inference-visualization-guide.md)를 본다. 사람 pose 영상 inference 비교는 [docs/pose-video-inference-guide.md](docs/pose-video-inference-guide.md)를 본다. COCO JSON 기반 전달용 dataset packaging은 [docs/coco-dataset-pack-guide.md](docs/coco-dataset-pack-guide.md)를 본다.
...
COCO JSON 기반 dataset packaging dry-run 예시:
...
실제 파일을 복사/저장하려면 `--run`을 추가한다.


YOLO11 pose 영상 inference dry-run 예시:
...
RF-DETR Keypoint 영상 inference dry-run 예시:
