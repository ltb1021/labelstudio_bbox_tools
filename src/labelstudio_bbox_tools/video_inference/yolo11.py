...
def _result_to_detections(result: Any, class_names: Sequence[str]) -> list[Detection]:
...
def run_yolo11_video_inference(
    *,
    input_path: str | Path,
    out_dir: str | Path,
    ...
    model_name = "yolo11"
    if dry_run:
        print(f"[dry-run] model={model_name}, weights={model_weights}, classes={len(class_names)}")
    ...
    def predict_frame(frame_bgr, frame_index: int, timestamp: float, video_path: Path | None) -> list[Detection]:
        ...
        if not results:
            return []
        return _result_to_detections(results[0], class_names)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run YOLO11 video inference and save bbox visualization videos.")
